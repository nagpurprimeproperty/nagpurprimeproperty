...
// ─── Property Listed By ───────────────────────────────────────────────────────
export type PropertyListedBy = 'Owner' | 'Broker' | 'Builder' | 'RERA Agent';
...
export interface PropertyWizardStore {
  // Navigation
  ...
  // Photo management (operates on step5.photos; upload cache lives in propertyUploadStore)
...
export const usePropertyWizardStore = create<PropertyWizardStore>((set, get) => ({
  currentStep: 1,
  ...
  // ─── Photo Management ──────────────────────────────────────────────────────
  // NOTE: removePhoto also needs to purge the upload cache in propertyUploadStore.
  // Call usePropertyUploadStore.getState().removeUploadedPhotoUrl(uri) alongside
  // this action when removing a photo from the wizard UI.
  ...
  buildSubmitPayload: (uploadedPhotos, uploadedVideoUrl) => {
    ...
    return {
      ...
      photos: uploadedPhotos ?? s.step5.photos,
  ...
  loadPropertyForEdit: (property, origin) => {
    ...
    const normalizePossessionTimelineValue = (val: any): string | undefined => {
    ...
    if (pricing.possessionTimeline) {
      pricing.possessionTimeline = normalizePossessionTimelineValue(pricing.possessionTimeline);
    }
...
export const selectCurrentCombo = (s: PropertyWizardStore) => ({
  category: s.step1.listingCategory,
  type: s.step1.propertyType,
});

export const selectPhotoCount = (s: PropertyWizardStore) => s.step5.photos.length;
export const selectAmenityCount = (s: PropertyWizardStore) => s.step6.amenities.length;
