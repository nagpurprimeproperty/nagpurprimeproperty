...
const DUMMY_IMAGES = Object.freeze([
  "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=800&q=80",
  "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&q=80",
  "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=800&q=80",
]);

const DUMMY_FALLBACK_IMAGES = Object.freeze([DUMMY_IMAGES[0], DUMMY_IMAGES[1], DUMMY_IMAGES[2]]);

const WINDOW_WIDTH = Dimensions.get("window").width;
export const PROPERTY_CARD_WIDTH_OFFSET = 24;
const DEFAULT_VERTICAL_WIDTH = Math.max(0, WINDOW_WIDTH - PROPERTY_CARD_WIDTH_OFFSET);
...
// ─── Pure Utility Functions (Hoisted outside render) ─────────────────────────

function resolveImages(item: PropertyCardItem): string[] {
  if (item.images && item.images.length > 0) return item.images;
  if (item.image) return [item.image, DUMMY_IMAGES[0], DUMMY_IMAGES[1]];
  return DUMMY_FALLBACK_IMAGES as unknown as string[];
}

function getIsNegotiable(item: PropertyCardItem): boolean {
  return Boolean(
...
function getVerticalSpecs(item: PropertyCardItem) {
  const details = item.details || {};
  const type = item.propertyType || item.type || "";
...
// ─── Self-Contained Heart Button Sub-component ─────────────────────────────────
...
// ─── Sub-Component: Horizontal Variant Card ───────────────────────────────────

const HorizontalCard = React.memo(function HorizontalCard({
  item,
  cardWidth,
  liked,
  colors,
  ...
  return (
    ...
    >
      ...
      <Pressable onPress={onCardPress} className="p-3">
        <Text
...
// ─── Sub-Component: Search List Variant Card ─────────────────────────────────

const SearchListCard = React.memo(function SearchListCard({
  item,
  cardWidth,
  liked,
  colors,
  ...
  const images = useMemo(() => resolveImages(item), [item.images, item.image]);
  const isNegotiable = useMemo(() => getIsNegotiable(item), [item]);
  const listSpecs = useMemo(() => getSearchListSpecs(item), [item]);
        ...
        <View style={{ position: "absolute", bottom: 8, left: 8, flexDirection: "row", gap: 4, zIndex: 10 }}>
          ...
          {videoCount > 0 && (
            <View style={{ backgroundColor: "rgba(0,0,0,0.6)", flexDirection: "row", alignItems: "center", paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4 }}>
              ...
              <Text style={{ color: "white", fontSize: 8, fontWeight: "800", marginLeft: 2 }}>{videoCount}</Text>
            </View>
          )}
        </View>
      </View>
...
// ─── Sub-Component: Vertical Variant Card ─────────────────────────────────────

const VerticalCard = React.memo(function VerticalCard({
  item,
  cardWidth,
  isScrollRow,
  liked,
  ...
  const images = useMemo(() => resolveImages(item), [item.images, item.image]);
  const isNegotiable = useMemo(() => getIsNegotiable(item), [item]);
  const specs = useMemo(() => getVerticalSpecs(item), [item]);
...
// ─── Main PropertyCard Component (Variant Dispatcher & Shared Hooks) ──────────
...
const PropertyCard = ({
  ...
  const cardWidth =
    widthProp ?? (variant === "horizontal" ? 200 : DEFAULT_VERTICAL_WIDTH);
  ...
  const handleLike = useCallback(() => {
    ...
    onToggleLike?.();
  }, [isAuthenticated, liked, propertyId, onToggleSave, onToggleLike, openAuth]);
