...
const PropertyCard = ({
  ...
  const handleLike = useCallback(() => {
    if (!isAuthenticated) {
      useAuthStore.getState().openAuth("saveProperty");
      return;
    }
    ...
    onToggleLike?.();
  }, [isAuthenticated, liked, propertyId, onToggleSave, onToggleLike, heartScale]);

  const handleCall = useCallback(async () => {
    if (!isAuthenticated) {
      useAuthStore.getState().openAuth("viewContact");
      return;
    }
  ...
  }, [isAuthenticated, propertyId, onCreateCallEnquiry, item.broker]);
  ...
  }, [isAuthenticated, propertyId, onCreateCallEnquiry, item.broker, item.title]);

  const handleShare = useCallback(() => {
    const locStr =
      typeof item.location === "string"
        ? item.location
        : item.location?.name ?? "Nagpur";
    Share.share({
