...
export const useNotifications = (page = 1, limit = 10) => {
  const result = useQuery({
    ...
    // but the base ['notifications'] prefix lets setQueriesData find all of them
...
export const useMarkNotificationAsReadMutation = () => {
  ...
  return useMutation({
    ...
    onMutate: async (notificationId: string) => {
      ...
      const previous = qc.getQueriesData({ queryKey: NOTIFICATIONS_KEY });
      qc.setQueriesData(
...
export const useUnreadCount = () => {
  return useQuery({
    queryKey: [...NOTIFICATIONS_KEY, { page: 1, limit: 10 }],
