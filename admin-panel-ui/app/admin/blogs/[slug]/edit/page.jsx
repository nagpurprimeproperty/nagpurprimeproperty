...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/v1/admin/blogs/${slug}`, {
  ...
  if (loading || !form) return <div className="p-8 text-muted-foreground text-sm">Loading blog…</div>;

  return (
    <PermissionGate module="blogs" action="write" fallback={<Unauthorized />}>
      <div className="space-y-6 w-full min-w-0">
        <AdminPageHeader title="Edit Blog" description={`Editing: ${form.title}`}>
          <a href={`${WEBSITE_BASE}/blogs/${slug}`} target="_blank" rel="noopener noreferrer">
            ...
            <CardContent className="grid gap-5 sm:grid-cols-2">
              ...
              <Field label="Slug" className="sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ⚠ Changing the slug will break existing links to this blog post.
                </p>
              </Field>
