...
const propertyRepository = {
  ...
  findAll: async (params = {}, userId) => {
    ...
    // Build a normalized cache key for the base listing
    const normalizedForCache = {
      ...
      locality: locality || '',
      latitude: params.latitude ? Number(params.latitude).toFixed(3) : '',
      longitude: params.longitude ? Number(params.longitude).toFixed(3) : '',
      featured: featured === true || featured === 'true' ? '1' : '0',
      isRecommended: isRecommended ? '1' : '0',
      isRelevanceSorted: isRelevanceSorted ? '1' : '0',
      priceSort: priceSort || '',
      budgetFrom: budgetFrom || '',
      budgetTo: budgetTo || '',
      amenities: [...amenitiesArray].sort(),
      bhk: bhk || '',
      isSaved: isSaved === true || isSaved === 'true' ? '1' : '0',
      brokerId: params.brokerId || '',
      userId: userObjectId ? String(userObjectId) : 'anon',
      page: Number(page),
      limit: safeLimit,
      status: params.status || 'Active',
    };

    const cacheKey = `property:list:${crypto.createHash('md5').update(JSON.stringify(normalizedForCache)).digest('hex')}`;
    ...
    const prePaginationStages = [
      ...
      // ── Saved check (pre-pagination so we can filter + count correctly) ──
      ...(userObjectId
        ? [
          {
            $lookup: {
 ...
 */
  ...
  findSimilarProperties: async (propertyId, userId, {
    ...
    const cacheKey = `property:similar:${crypto.createHash('md5').update(JSON.stringify(normalizedForCache)).digest('hex')}`;
